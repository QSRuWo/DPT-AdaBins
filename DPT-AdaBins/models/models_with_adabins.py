...
class DPTWithAdaBins(BaseModel):
    def __init__(
        ...
        self.conv_out = nn.Sequential(nn.Conv2d(128, n_bins, kernel_size=1, stride=1, padding=0),
                                      ...
                                      # 尝试在这里加入一个双线性插值,暂时去掉
                                      nn.Softmax(dim=1))  # 举例：如果输入张量形状为[2, 100, 240, 320]，这里使得每一个批次每一个像素点的100个值之和等于1
        self.bn_test = nn.BatchNorm2d(num_features=128)
    ...
    def forward(self, x):
        ...
        '''测试'''
        # range_attention_maps = torch.tanh(range_attention_maps)
        # range_attention_maps = torch.clamp(range_attention_maps, min=-1, max=1)
        range_attention_maps = self.bn_test(range_attention_maps)
