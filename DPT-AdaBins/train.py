...
from models.models_with_adabins import DPTDepthModelWithAdaBins
...
def main_worker(gpu, ngpus_per_node, args):
    args.gpu = gpu
    print(f"gpu in main_worker: {gpu}")
    ...
    if args.model_type == "original":
        ...
        if args.model_type == "hybrid":
            ...
            pretrained_weights = torch.load("./weights/nyu_merged_weights.pt")
            ...
            pretrained_weights = torch.load("./weights/nyu_merged_weights_large.pt")
        elif args.model_type == "dpt-only":
            model = DPTDepthModelWithAdaBins(
                path=None,
                backbone="vitb_rn50_384",
                non_negative=True,
            ...
            # 记得修改
            pretrained_weights = torch.load("./weights/nyu_merged_weights.pt")
        elif args.model_type == "adabins-only":
            model = DPTDepthModelWithAdaBins(
                path=None,
                backbone="vitb_rn50_384",
                non_negative=True,
    ...
    if args.fine_tune:
        for name, parameter in model.named_parameters():
            if name.startswith("scratch") or name.startswith("conv_out") \
                or name.startswith("conv1x1") or name.startswith("bn"):
    ...
    ################################################################################################
    print(f"args.gpu before set_device: {args.gpu}")
...
def train(model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    print(f"len of train_loader: {len(train_loader.dataset)}")
    ...
    if args.same_lr:
        ...
        for name, param in m.named_parameters():
            if "pretrained" in name:
                ...
                adabins_params.append(param)
        params = [
            {"params": pretrained_params, 'lr':1e-5},
            {"params": scratch_params, 'lr':1e-4},
            {"params": adabins_params, 'lr':lr}
        ]



    # optimizer = optim.AdamW(params, weight_decay=args.wd, lr=args.lr)
    # optimizer = optim.Adam(params, weight_decay=args.wd, lr=args.lr)
    optimizer = optim.SGD(params, weight_decay=args.wd, lr=args.lr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', '--learning-rate', default=0.0000357, type=float, help='max learning rate')
    ...
    parser.add_argument('--model_type', default="hybrid", type=str, help="original, hybrid, large, dpt-only, adabins-only")
